		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMRSV1JobExists("telefonicaopencloud_mrs_job_v1.job1", &jobGet),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_mrs_job_v1.job1", "job_state", "Completed"),
				),
			},
		},
	})
...
func testAccCheckMRSV1JobDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating telefonicaopencloud mrs: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_mrs_job_v1" {
			continue
		}
...
resource "telefonicaopencloud_mrs_job_v1" "job1" {
  ...
  cluster_id = "${telefonicaopencloud_mrs_cluster_v1.cluster1.id}"
