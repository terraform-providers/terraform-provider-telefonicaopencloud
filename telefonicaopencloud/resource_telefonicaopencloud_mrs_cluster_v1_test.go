		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMRSV1ClusterExists("telefonicaopencloud_mrs_cluster_v1.cluster1", &clusterGet),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_mrs_cluster_v1.cluster1", "cluster_state", "running"),
				),
			},
		},
	})
...
func testAccCheckMRSV1ClusterDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_mrs_cluster_v1" {
			continue
		}
